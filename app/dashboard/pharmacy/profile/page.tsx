...
import { usePharmacyProfile, useUpdatePharmacyProfile, useAuditLogs } from "@/hooks/queries"
...
type Section = "menu" | "Pharmacy Details" | "Activity Log" | "Security & Settings"
...
export default function PharmacyProfilePage() {
    ...
    const { data: profile, isPending } = usePharmacyProfile()
    const { mutate: updateProfile, isPending: isUpdating } = useUpdatePharmacyProfile()
    ...
    const { register, handleSubmit, reset } = useForm<PharmacyFormData>()
    ...
    const displayName = profile?.pharmacy_name || user?.name || user?.email?.split("@")[0] || "Pharmacy"
    ...
    const desktopLayout = (
        <div className="hidden md:block max-w-5xl mx-auto pb-12">
            <div className="mb-8">
                ...
                <p className="text-sm text-muted-foreground mt-1">Manage your retail details and account settings.</p>
            </div>
            <div className="flex gap-8">
                ...
                <div className="w-72 shrink-0 space-y-6">
                    ...
                    <div className="flex flex-col gap-1.5">
                        {([
                            { label: "Pharmacy Details", icon: Medicine01Icon },
                ...
                <div className="flex-1">
                    <div className="bg-card border border-border rounded-[32px] p-8 min-h-[500px]">
                        {section === "Pharmacy Details" && <PersonalInfoForm {...{ profile, isPending, isUpdating, isUploadingPhoto, uploadProgress, file, handlePhotoChange, handleSubmit, onSubmit, register }} />}
    ...
    const mobileMenu = (
        <div className="md:hidden pb-4">
            ...
            <div className="space-y-2">
                {([
                    { label: "Pharmacy Details", icon: Building01Icon, desc: "Store name, address, license" },
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
            <div className="flex flex-col sm:flex-row gap-6 items-start sm:items-center pb-8 border-b border-border">
                ...
                <div className="flex-1 space-y-1">
                    <h3 className="font-semibold text-sm text-foreground">Store Logo</h3>
                    <p className="text-xs text-muted-foreground max-w-sm leading-relaxed">
                        Upload your pharmacy's logo or verified insignia. Changes save instantly.
                    </p>
                    {isUploadingPhoto && (
            ...
            <div className="grid gap-5">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div className="grid gap-2">
                        <Label htmlFor="pharmacy_name" className="text-muted-foreground text-xs font-bold uppercase tracking-wider">Pharmacy Name</Label>
                        <Input id="pharmacy_name" className="h-12 rounded-xl bg-muted/40 border-transparent focus-visible:bg-transparent focus-visible:ring-2 focus-visible:ring-primary shadow-sm" placeholder="Green Cross Pharmacy" {...register("pharmacy_name")} disabled={isPending} />
                    </div>
                    <div className="grid gap-2">
                        ...
                        <Input id="license_number" className="h-12 rounded-xl bg-muted/40 border-transparent focus-visible:bg-transparent focus-visible:ring-2 focus-visible:ring-primary shadow-sm" placeholder="PHARM-..." {...register("license_number")} disabled={isPending} />
                    </div>
                </div>
                ...
                <div className="grid gap-2">
                    <Label htmlFor="address" className="text-muted-foreground text-xs font-bold uppercase tracking-wider">Store Address</Label>
                    <Textarea id="address" className="min-h-[100px] rounded-xl bg-muted/40 border-transparent focus-visible:bg-transparent focus-visible:ring-2 focus-visible:ring-primary shadow-sm" placeholder="45 Retail Blvd..." {...register("address")} disabled={isPending} />
                </div>
            </div>
