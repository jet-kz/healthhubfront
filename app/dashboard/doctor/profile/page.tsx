...
import { useDoctorProfile, useUpdateDoctorProfile, useAuditLogs } from "@/hooks/queries"
...
type Section = "menu" | "Professional Info" | "Activity Log" | "Security & Settings"
...
export default function DoctorProfilePage() {
    ...
    const { data: profile, isPending } = useDoctorProfile()
    ...
    const displayName = profile?.full_name || user?.name || user?.email?.split("@")[0] || "Doctor"
    ...
    const desktopLayout = (
        <div className="hidden md:block max-w-5xl mx-auto pb-12">
            <div className="mb-8">
                ...
                <p className="text-sm text-muted-foreground mt-1">Manage your professional profile and account settings.</p>
            </div>
            <div className="flex gap-8">
                ...
                <div className="w-72 shrink-0 space-y-6">
                    <div className="flex items-center gap-4 p-4 rounded-3xl bg-card border border-border">
                        ...
                        <div className="min-w-0">
                            <p className="font-bold text-sm text-foreground truncate">Dr. {displayName?.split(" ")?.[displayName?.split(" ").length - 1] || displayName}</p>
                            <p className="text-xs text-muted-foreground truncate">{displayEmail}</p>
                        </div>
                    </div>
                    <div className="flex flex-col gap-1.5">
                        {([
                            { label: "Professional Info", icon: UserIcon },
                ...
                <div className="flex-1">
                    <div className="bg-card border border-border rounded-[32px] p-8 min-h-[500px]">
                        {section === "Professional Info" && <PersonalInfoForm {...{ profile, isPending, isUpdating, isUploadingPhoto, uploadProgress, file, handlePhotoChange, handleSubmit, onSubmit, register }} />}
    ...
    const mobileMenu = (
        <div className="md:hidden pb-4">
            ...
            <div className="flex flex-col items-center pt-4 pb-8">
                ...
                <h2 className="text-xl font-bold text-foreground">Dr. {displayName?.split(" ")?.[displayName?.split(" ").length - 1] || displayName}</h2>
            ...
            <div className="space-y-2">
                {([
                    { label: "Professional Info", icon: UserIcon, desc: "Name, specialty, address, fee" },
            ...
            <div className="flex flex-col sm:flex-row gap-6 items-start sm:items-center pb-8 border-b border-border">
                ...
                <div className="flex-1 space-y-1">
                    ...
                    <p className="text-xs text-muted-foreground max-w-sm leading-relaxed">
                        Upload a clear photo for your patients to see. Changes save instantly.
                    </p>
                    {isUploadingPhoto && (
            ...
            <div className="grid gap-5">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div className="grid gap-2">
                        ...
                        <Input id="full_name" className="h-12 rounded-xl bg-muted/40 border-transparent focus-visible:bg-transparent focus-visible:ring-2 focus-visible:ring-primary shadow-sm" placeholder="Dr. John Doe" {...register("full_name")} disabled={isPending} />
                    </div>
                    <div className="grid gap-2">
                        <Label htmlFor="specialization" className="text-muted-foreground text-xs font-bold uppercase tracking-wider">Specialization</Label>
                        <Input id="specialization" className="h-12 rounded-xl bg-muted/40 border-transparent focus-visible:bg-transparent focus-visible:ring-2 focus-visible:ring-primary shadow-sm" placeholder="Cardiologist" {...register("specialization")} disabled={isPending} />
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div className="grid gap-2">
                        ...
                        <Input id="license_number" className="h-12 rounded-xl bg-muted/40 border-transparent focus-visible:bg-transparent focus-visible:ring-2 focus-visible:ring-primary shadow-sm" placeholder="MD-..." {...register("license_number")} disabled={isPending} />
                    </div>
                    <div className="grid gap-2">
                        <Label htmlFor="consultation_fee" className="text-muted-foreground text-xs font-bold uppercase tracking-wider">Consultation Fee ($)</Label>
                        <Input id="consultation_fee" type="number" step="0.01" className="h-12 rounded-xl bg-muted/40 border-transparent focus-visible:bg-transparent focus-visible:ring-2 focus-visible:ring-primary shadow-sm" placeholder="100.00" {...register("consultation_fee", { valueAsNumber: true })} disabled={isPending} />
                    </div>
                </div>
                ...
                <div className="grid gap-2">
                    <Label htmlFor="bio" className="text-muted-foreground text-xs font-bold uppercase tracking-wider">Professional Bio</Label>
                    <Textarea id="bio" className="min-h-[100px] rounded-xl bg-muted/40 border-transparent focus-visible:bg-transparent focus-visible:ring-2 focus-visible:ring-primary shadow-sm" placeholder="Tell your patients about your experience and expertise..." {...register("bio")} disabled={isPending} />
                </div>
            </div>
